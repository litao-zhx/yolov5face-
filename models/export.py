...
#     parser.add_argument('--weights', type=str, default='/data/lt/yolov5-face/runs/train2/exp/weights/last.pt', help='weights path')  # from yolov5/models/
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', type=str, default='/data/lt/yolov5-face/runs/train2/exp2/weights/best.pt',
                        help='weights path')  # from yolov5/models/
    # parser.add_argument('--weights', type=str, default='/data/lt/yolov5-face/runs/train/exp8/weights/best.pt', help='weights path')  # from yolov5/models/
    parser.add_argument('--img_size', nargs='+', type=int, default=[640], help='image size')  # height, width
    parser.add_argument('--batch_size', type=int, default=1, help='batch size')
    parser.add_argument('--onnx2pb', action='store_true', default=False, help='export onnx to pb')
    ...
    for k, m in model.named_modules():
        ...
        # ShuffleV2Block这个模块在yolov5n的版本里使用
        if isinstance(m, models.common.ShuffleV2Block):  # shufflenet block nn.SiLU
            for i in range(len(m.branch1)):
                if isinstance(m.branch1[i], nn.SiLU):
                    m.branch1[i] = SiLU()
    ...
    input_names = ['input']
    output_names = ['output']
    torch.onnx.export(model, img, f, verbose=False, opset_version=11, input_names=input_names,
    ...
    if opt.onnx2trt:
        print('\nStarting TensorRT...')
        # ONNX_to_TensorRT(onnx_model_path=f, trt_engine_path=f.replace('.onnx', '.trt'), fp16_mode=opt.fp16_trt)
